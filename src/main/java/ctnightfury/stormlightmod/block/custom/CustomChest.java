...
public class CustomChest extends AbstractChestBlock<ChestBlockEntity> implements Waterloggable {
    public static final MapCodec<CustomChest> CODEC = createCodec(settings -> new CustomChest(settings, () -> BlockEntityType.CHEST));
    ...
    private static final DoubleBlockProperties.PropertyRetriever<ChestBlockEntity, Optional<Inventory>> INVENTORY_RETRIEVER = new DoubleBlockProperties.PropertyRetriever<>() {
    ...
    @Override
    public MapCodec<? extends CustomChest> getCodec() {
        return CODEC;
    }

    public CustomChest(AbstractBlock.Settings settings, Supplier<BlockEntityType<? extends ChestBlockEntity>> supplier) {
        super(settings, supplier);
        this.setDefaultState(
                this.stateManager.getDefaultState().with(FACING, Direction.NORTH).with(CHEST_TYPE, ChestType.SINGLE).with(WATERLOGGED, Boolean.FALSE)
        );
    }
    ...
    @Nullable
    public static Inventory getInventory(CustomChest block, BlockState state, World world, BlockPos pos, boolean ignoreBlocked) {
